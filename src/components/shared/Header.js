...
const unauthenticatedOptions = (
	<>
		<Nav.Item>
			<Link className="nav-items m-3" to='sign-up' style={linkStyle}>Sign Up</Link>
		</Nav.Item>
		<Nav.Item>
			<Link className="nav-items m-3" to='/' style={linkStyle}>Sign In</Link>
		</Nav.Item>
	</>
)
...
const alwaysOptions = (
	<>
		<Nav.Link>
			<Link className="nav-items m-2" to='/outfits' style={linkStyle}>
				Fit Feed
			</Link>
		</Nav.Link>
	</>
	...
	<Navbar expand='md' className='nav-bar'>
		...
		<Navbar.Collapse id='basic-navbar-nav'>
			<Nav className='ml-auto'>
				{user && (
					<span className='nav-username m-3'>{user.email.split('@')[0]}</span>
				)}
				{alwaysOptions}
